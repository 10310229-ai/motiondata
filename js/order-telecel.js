  ...
  telecelForm.addEventListener('submit', function(evt){
    ...
    const handler = PaystackPop.setup({
      ...
      callback: function(response){
        try {
          const order = { id: response.reference, reference: response.reference, date: new Date().toISOString(), timestamp: Date.now(), email: email, phone: msisdn, mobile: phone, operator: 'Telecel', network: 'Telecel', package: pkg, amount: price, status: 'completed' };
